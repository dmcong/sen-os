...
import { Row, Col, Button, Card, Typography, Icon, Input, Space, Tooltip } from 'sen-kit';
...
const Transfer = ({ accountData, onChange }) => {
  ...
  const transfer = async () => {
    try {
      ...
      await notify({
        type: 'success',
        description: `Transfer ${srcValue} SOL to ${dstValue.substring(0, 6) + '...' + dstValue.substring(dstValue.length - 6, dstValue.length)} successfully. Click to view details.`,
  ...
  const icon = () => {
    if (!ssjs.isAddress(dstValue)) return <Tooltip title="Invalid address">
      <Icon name='warning-outline' style={{ color: '#F2323F' }} />
    </Tooltip>
    return <Tooltip title={`The account is safe to transfer.`}>
